...
####################### Scenario 14: SKEW NORMAL DATA, k=26, mu=0, tau2= 2.63 #######################
...
params <- cp2dp(c(0 , sqrt(2.63), 0.785), "SN")
...
#studies_zero_2arms
select <- which(p[[studies_zero_2arms]]$ti == 0 & p[[studies_zero_2arms]]$ci == 0)
p[[studies_zero_2arms]] = p[[studies_zero_2arms]][-select, ]
...
############ TO EXTRACT THE RELATIVE EFFECTS ######
max_rowsNU <- max(sapply(listaU, function(x) dim(x)[1]))
# 
common_colnamesNU <- colnames(listaU[[1]])  
# 
for (i in 2:length(listaU)) {
  if (!identical(colnames(listaU[[i]]), common_colnamesNU)) {
    stop("Column names do not match between lists.")
  }
}

for (i in 1:length(listaU)) {
  current_rowsNU <- dim(listaU[[i]])[1]
  if (current_rowsNU < max_rowsNU) {
    extra_rowsNU <- max_rowsNU - current_rowsNU
    extra_dataNU <- matrix(NA, nrow = extra_rowsNU, ncol = ncol(listaU[[i]]))
    colnames(extra_dataNU) <- common_colnamesNU 
    listaU[[i]] <- rbind(listaU[[i]], extra_dataNU)
  } else if (current_rowsNU > max_rowsNU) {
    listaU[[i]] <- listaU[[i]][1:max_rowsNU, ]
  }
}
...
####### NEED TO FIND THE DATASET WITH THE STUDY WITH 0-0 TWO ARMS ######
subtrU <- length(which(dist.condNU < studies_zero_2arms))
numberU = studies_zero_2arms - subtrU

listaNorm_overlapU = listaNormU[-numberU]  
p_overlapU = pNU[-numberU]
N.sim = length(listaNorm_overlapU)

abs_bias_relNU <- list()
mean_abs_bias_relNU <- list()
for(i in 1:N.sim){
  for(j in 1:nrow(pNU[[i]])){
    abs_bias_relNU[[i]] <- round(abs( listaNorm_overlapU[[i]]$`rel_effU[[i]]` - p_overlapU[[i]]$true_eff  ),2)
...
for(i in 1:N.sim){
  for(j in 1:nrow(pNU[[i]])){
    rel_bias_relNU[[i]] <- round(( listaNorm_overlapU[[i]]$`rel_effU[[i]]` - p_overlapU[[i]]$true_eff  ),2)
...
overlapNU <- list()
for(i in 1:N.sim){
  dataNU[[i]] <- list(NORMAL = listaNorm_overlapU[[i]]$`rel_effU[[i]]` , True = p_overlapU[[i]]$true_eff)
...
write.csv(Norm_bin_jagsU, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\NORMAL MODEL\\rev_UNIF_ResultsNormal_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(listaNormU, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\NORMAL MODEL\\rev_UNIF_Relative_eff_ResultsNormal_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(lista_rel_effU, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\NORMAL MODEL\\rev_UNIF_Bias_Relative_eff_ResultsNormal_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(Overalp_NormalU, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\NORMAL MODEL\\rev_UNIF_overalp_Normal_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
...
############ TO EXTRACT THE RELATIVE EFFECTS ######
max_rowsNHN <- max(sapply(listaHN, function(x) dim(x)[1]))
# 
common_colnamesNHN <- colnames(listaHN[[1]])  
# 
for (i in 2:length(listaHN)) {
  if (!identical(colnames(listaHN[[i]]), common_colnamesNHN)) {
    stop("Column names do not match between lists.")
  }
}

for (i in 1:length(listaHN)) {
  current_rowsNHN <- dim(listaHN[[i]])[1]
  if (current_rowsNHN < max_rowsNHN) {
    extra_rowsNHN <- max_rowsNHN - current_rowsNHN
    extra_dataNHN <- matrix(NA, nrow = extra_rowsNHN, ncol = ncol(listaHN[[i]]))
    colnames(extra_dataNHN) <- common_colnamesNHN 
    listaHN[[i]] <- rbind(listaHN[[i]], extra_dataNHN)
  } else if (current_rowsNHN > max_rowsNHN) {
    listaHN[[i]] <- listaHN[[i]][1:max_rowsNHN, ]
  }
}
...
######################## ABSOLUTE BIAS OF RELATIVE EFFECTS ##################
####### NEED TO FIND THE DATASET WITH THE STUDY WITH 0-0 TWO ARMS ######
subtrHN <- length(which(dist.condNHN < studies_zero_2arms))
numberHN = studies_zero_2arms - subtrHN

listaNorm_overlapHN = listaNormHN[-numberHN]  
p_overlapHN = pNHN[-numberHN]
N.sim = length(listaNorm_overlapHN)

abs_bias_relNHN <- list()
mean_abs_bias_relNHN <- list()
for(i in 1:N.sim){
  for(j in 1:nrow(pNHN[[i]])){
    abs_bias_relNHN[[i]] <- round(abs( listaNorm_overlapHN[[i]]$`rel_effHN[[i]]` - p_overlapHN[[i]]$true_eff  ),2)
...
for(i in 1:N.sim){
  for(j in 1:nrow(pNHN[[i]])){
    rel_bias_relNHN[[i]] <- round(( listaNorm_overlapHN[[i]]$`rel_effHN[[i]]` - p_overlapHN[[i]]$true_eff  ),2)
...
overlapNHN <- list()
for(i in 1:N.sim){
  dataNHN[[i]] <- list(NORMAL = listaNorm_overlapHN[[i]]$`rel_effHN[[i]]` , True = p_overlapHN[[i]]$true_eff)
...
write.csv(Norm_bin_jagsHN, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\NORMAL MODEL\\rev_HN_ResultsNormal_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(listaNormHN, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\NORMAL MODEL\\rev_HN_Relative_eff_ResultsNormal_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(lista_rel_effHN, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\NORMAL MODEL\\rev_HN_Bias_Relative_eff_ResultsNormal_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(Overalp_NormalHN, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\NORMAL MODEL\\rev_HN_overalp_Normal_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
...
# ############ TO EXTRACT THE RELATIVE EFFECTS ####
max_rowsDP_HN_U_26 <- max(sapply(listaDP1_HN_U_26, function(x) dim(x)[1]))
common_colnamesDP_HN_U_26 <- colnames(listaDP1_HN_U_26[[1]])  
# 
for (i in 2:length(listaDP1_HN_U_26)) {
  if (!identical(colnames(listaDP1_HN_U_26[[i]]), common_colnamesDP_HN_U_26)) {
    stop("Column names do not match between lists.")
  }
}
for (i in 1:length(listaDP1_HN_U_26)) {
  current_rowsDP_HN_U_26 <- dim(listaDP1_HN_U_26[[i]])[1]
  if (current_rowsDP_HN_U_26 < max_rowsDP_HN_U_26) {
    extra_rowsDP_HN_U_26 <- max_rowsDP_HN_U_26 - current_rowsDP_HN_U_26
    extra_dataDP_HN_U_26 <- matrix(NA, nrow = extra_rowsDP_HN_U_26, ncol = ncol(listaDP1_HN_U_26[[i]]))
    colnames(extra_dataDP_HN_U_26) <- common_colnamesDP_HN_U_26  
    listaDP1_HN_U_26[[i]] <- rbind(listaDP1_HN_U_26[[i]], extra_dataDP_HN_U_26)
  } else if (current_rowsDP_HN_U_26 > max_rowsDP_HN_U_26) {
    listaDP1_HN_U_26[[i]] <- listaDP1_HN_U_26[[i]][1:max_rowsDP_HN_U_26, ]
  }
}
# 
max_rowsDPw_HN_U_26 <- max(sapply(listaDP_weights_HN_U_26, function(x) dim(x)[1]))
common_colnamesDPw_HN_U_26 <- colnames(listaDP_weights_HN_U_26[[1]])  
# 
for (i in 2:length(listaDP_weights_HN_U_26)) {
  if (!identical(colnames(listaDP_weights_HN_U_26[[i]]), common_colnamesDPw_HN_U_26)) {
    stop("Column names do not match between lists.")
  }
}
for (i in 1:length(listaDP_weights_HN_U_26)) {
  current_rowsDPw_HN_U_26 <- dim(listaDP_weights_HN_U_26[[i]])[1]
  if (current_rowsDPw_HN_U_26 < max_rowsDPw_HN_U_26) {
    extra_rowsDPw_HN_U_26 <- max_rowsDPw_HN_U_26 - current_rowsDPw_HN_U_26
    extra_dataDPw_HN_U_26 <- matrix(NA, nrow = extra_rowsDPw_HN_U_26, ncol = ncol(listaDP_weights_HN_U_26[[i]]))
    colnames(extra_dataDPw_HN_U_26) <- common_colnamesDPw_HN_U_26  
    listaDP_weights_HN_U_26[[i]] <- rbind(listaDP_weights_HN_U_26[[i]], extra_dataDPw_HN_U_26)
  } else if (current_rowsDPw_HN_U_26 > max_rowsDPw_HN_U_26) {
    listaDP_weights_HN_U_26[[i]] <- listaDP_weights_HN_U_26[[i]][1:max_rowsDPw_HN_U_26, ]
  }
}
...
subtr_HN_U_26 <- length(which(dist.condDP_HN_U_26 < studies_zero_2arms))
number_HN_U_26 = studies_zero_2arms - subtr_HN_U_26


listaDP_overlap_HN_U_26 = listaDP_HN_U_26[-number_HN_U_26]  
p_overlapDP_HN_U_26 = pDP_HN_U_26[-number_HN_U_26]
N.sim = length(listaDP_overlap_HN_U_26)

abs_bias_relDP_HN_U_26 <- list()
mean_abs_bias_relDP_HN_U_26 <- list()
for(i in 1:N.sim){
  for(j in 1:nrow(pDP_HN_U_26[[i]])){
    abs_bias_relDP_HN_U_26[[i]] <- round(abs(listaDP_overlap_HN_U_26[[i]]$`rel_effDP_HN_U_26[[i]]` - p_overlapDP_HN_U_26[[i]]$true_eff ),2)
...
for(i in 1:N.sim){
  for(j in 1:nrow(pDP_HN_U_26[[i]])){
    rel_bias_relDP_HN_U_26[[i]] <- round((listaDP_overlap_HN_U_26[[i]]$`rel_effDP_HN_U_26[[i]]` - p_overlapDP_HN_U_26[[i]]$true_eff ),2)
...
for(i in 1:N.sim){
  dataDP_HN_U_26[[i]] <- list(DP = listaDP_overlap_HN_U_26[[i]]$`rel_effDP_HN_U_26[[i]]` , True = p_overlapDP_HN_U_26[[i]]$true_eff)
...
write.csv(DP_bin_jags_HN_U_26, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_HN_UNIF_26_ResultsDP_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(listaDP_HN_U_26, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_HN_UNIF_26_Relative_eff_ResultsDP_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(listaDP_rel_eff_HN_U_26, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_HN_UNIF_26_Bias_Relative_eff_ResultsDP_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(listaDP_weights_HN_U_26, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_HN_UNIF_26_Weights_in_clusters_DP_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=TRUE )  
write.csv(Overlap_DPmodel_HN_U_26, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_HN_UNIF_26_Overlap_DPmodel_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=TRUE )  
...
# ############ TO EXTRACT THE RELATIVE EFFECTS ####
max_rowsDP_HN_U_51 <- max(sapply(listaDP1_HN_U_51, function(x) dim(x)[1]))
common_colnamesDP_HN_U_51 <- colnames(listaDP1_HN_U_51[[1]])  
# 
for (i in 2:length(listaDP1_HN_U_51)) {
  if (!identical(colnames(listaDP1_HN_U_51[[i]]), common_colnamesDP_HN_U_51)) {
    stop("Column names do not match between lists.")
  }
}
for (i in 1:length(listaDP1_HN_U_51)) {
  current_rowsDP_HN_U_51 <- dim(listaDP1_HN_U_51[[i]])[1]
  if (current_rowsDP_HN_U_51 < max_rowsDP_HN_U_51) {
    extra_rowsDP_HN_U_51 <- max_rowsDP_HN_U_51 - current_rowsDP_HN_U_51
    extra_dataDP_HN_U_51 <- matrix(NA, nrow = extra_rowsDP_HN_U_51, ncol = ncol(listaDP1_HN_U_51[[i]]))
    colnames(extra_dataDP_HN_U_51) <- common_colnamesDP_HN_U_51  
    listaDP1_HN_U_51[[i]] <- rbind(listaDP1_HN_U_51[[i]], extra_dataDP_HN_U_51)
  } else if (current_rowsDP_HN_U_51 > max_rowsDP_HN_U_51) {
    listaDP1_HN_U_51[[i]] <- listaDP1_HN_U_51[[i]][1:max_rowsDP_HN_U_51, ]
  }
}
# 
max_rowsDPw_HN_U_51 <- max(sapply(listaDP_weights_HN_U_51, function(x) dim(x)[1]))
common_colnamesDPw_HN_U_51 <- colnames(listaDP_weights_HN_U_51[[1]])  
# 
for (i in 2:length(listaDP_weights_HN_U_51)) {
  if (!identical(colnames(listaDP_weights_HN_U_51[[i]]), common_colnamesDPw_HN_U_51)) {
    stop("Column names do not match between lists.")
  }
}
for (i in 1:length(listaDP_weights_HN_U_51)) {
  current_rowsDPw_HN_U_51 <- dim(listaDP_weights_HN_U_51[[i]])[1]
  if (current_rowsDPw_HN_U_51 < max_rowsDPw_HN_U_51) {
    extra_rowsDPw_HN_U_51 <- max_rowsDPw_HN_U_51 - current_rowsDPw_HN_U_51
    extra_dataDPw_HN_U_51 <- matrix(NA, nrow = extra_rowsDPw_HN_U_51, ncol = ncol(listaDP_weights_HN_U_51[[i]]))
    colnames(extra_dataDPw_HN_U_51) <- common_colnamesDPw_HN_U_51  
    listaDP_weights_HN_U_51[[i]] <- rbind(listaDP_weights_HN_U_51[[i]], extra_dataDPw_HN_U_51)
  } else if (current_rowsDPw_HN_U_51 > max_rowsDPw_HN_U_51) {
    listaDP_weights_HN_U_51[[i]] <- listaDP_weights_HN_U_51[[i]][1:max_rowsDPw_HN_U_51, ]
  }
}
...
subtr_HN_U_51 <- length(which(dist.condDP_HN_U_51 < studies_zero_2arms))
number_HN_U_51 = studies_zero_2arms - subtr_HN_U_51


listaDP_overlap_HN_U_51 = listaDP_HN_U_51[-number_HN_U_51]  
p_overlapDP_HN_U_51 = pDP_HN_U_51[-number_HN_U_51]
N.sim = length(listaDP_overlap_HN_U_51)

abs_bias_relDP_HN_U_51 <- list()
mean_abs_bias_relDP_HN_U_51 <- list()
for(i in 1:N.sim){
  for(j in 1:nrow(pDP_HN_U_51[[i]])){
    abs_bias_relDP_HN_U_51[[i]] <- round(abs(listaDP_overlap_HN_U_51[[i]]$`rel_effDP_HN_U_51[[i]]` - p_overlapDP_HN_U_51[[i]]$true_eff ),2)
...
for(i in 1:N.sim){
  for(j in 1:nrow(pDP_HN_U_51[[i]])){
    rel_bias_relDP_HN_U_51[[i]] <- round((listaDP_overlap_HN_U_51[[i]]$`rel_effDP_HN_U_51[[i]]` - p_overlapDP_HN_U_51[[i]]$true_eff ),2)
...
for(i in 1:N.sim){
  dataDP_HN_U_51[[i]] <- list(DP = listaDP_overlap_HN_U_51[[i]]$`rel_effDP_HN_U_51[[i]]` , True = p_overlapDP_HN_U_51[[i]]$true_eff)
...
write.csv(DP_bin_jags_HN_U_51, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_HF_U51_ResultsDP_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(listaDP_HN_U_51, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_HF_U51_Relative_eff_ResultsDP_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(listaDP_rel_eff_HN_U_51, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_HF_U51_Bias_Relative_eff_ResultsDP_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(listaDP_weights_HN_U_51, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_HF_U51_Weights_in_clusters_DP_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=TRUE )  
write.csv(Overlap_DPmodel_HN_U_51, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_HF_U51_Overlap_DPmodel_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=TRUE )  
...
# ############ TO EXTRACT THE RELATIVE EFFECTS ####
max_rowsDP_U_U_26 <- max(sapply(listaDP1_U_U_26, function(x) dim(x)[1]))
common_colnamesDP_U_U_26 <- colnames(listaDP1_U_U_26[[1]])  
# 
for (i in 2:length(listaDP1_U_U_26)) {
  if (!identical(colnames(listaDP1_U_U_26[[i]]), common_colnamesDP_U_U_26)) {
    stop("Column names do not match between lists.")
  }
}
for (i in 1:length(listaDP1_U_U_26)) {
  current_rowsDP_U_U_26 <- dim(listaDP1_U_U_26[[i]])[1]
  if (current_rowsDP_U_U_26 < max_rowsDP_U_U_26) {
    extra_rowsDP_U_U_26 <- max_rowsDP_U_U_26 - current_rowsDP_U_U_26
    extra_dataDP_U_U_26 <- matrix(NA, nrow = extra_rowsDP_U_U_26, ncol = ncol(listaDP1_U_U_26[[i]]))
    colnames(extra_dataDP_U_U_26) <- common_colnamesDP_U_U_26  
    listaDP1_U_U_26[[i]] <- rbind(listaDP1_U_U_26[[i]], extra_dataDP_U_U_26)
  } else if (current_rowsDP_U_U_26 > max_rowsDP_U_U_26) {
    listaDP1_U_U_26[[i]] <- listaDP1_U_U_26[[i]][1:max_rowsDP_U_U_26, ]
  }
}
# 
max_rowsDPw_U_U_26 <- max(sapply(listaDP_weights_U_U_26, function(x) dim(x)[1]))
common_colnamesDPw_U_U_26 <- colnames(listaDP_weights_U_U_26[[1]])  
# 
for (i in 2:length(listaDP_weights_U_U_26)) {
  if (!identical(colnames(listaDP_weights_U_U_26[[i]]), common_colnamesDPw_U_U_26)) {
    stop("Column names do not match between lists.")
  }
}
for (i in 1:length(listaDP_weights_U_U_26)) {
  current_rowsDPw_U_U_26 <- dim(listaDP_weights_U_U_26[[i]])[1]
  if (current_rowsDPw_U_U_26 < max_rowsDPw_U_U_26) {
    extra_rowsDPw_U_U_26 <- max_rowsDPw_U_U_26 - current_rowsDPw_U_U_26
    extra_dataDPw_U_U_26 <- matrix(NA, nrow = extra_rowsDPw_U_U_26, ncol = ncol(listaDP_weights_U_U_26[[i]]))
    colnames(extra_dataDPw_U_U_26) <- common_colnamesDPw_U_U_26  
    listaDP_weights_U_U_26[[i]] <- rbind(listaDP_weights_U_U_26[[i]], extra_dataDPw_U_U_26)
  } else if (current_rowsDPw_U_U_26 > max_rowsDPw_U_U_26) {
    listaDP_weights_U_U_26[[i]] <- listaDP_weights_U_U_26[[i]][1:max_rowsDPw_U_U_26, ]
  }
}
...
subtr_U_U_26 <- length(which(dist.condDP_U_U_26 < studies_zero_2arms))
number_U_U_26 = studies_zero_2arms - subtr_U_U_26


listaDP_overlap_U_U_26 = listaDP_U_U_26[-number_U_U_26]  
p_overlapDP_U_U_26 = pDP_U_U_26[-number_U_U_26]
N.sim = length(listaDP_overlap_U_U_26)

abs_bias_relDP_U_U_26 <- list()
mean_abs_bias_relDP_U_U_26 <- list()
for(i in 1:N.sim){
  for(j in 1:nrow(pDP_U_U_26[[i]])){
    abs_bias_relDP_U_U_26[[i]] <- round(abs(listaDP_overlap_U_U_26[[i]]$`rel_effDP_U_U_26[[i]]` - p_overlapDP_U_U_26[[i]]$true_eff ),2)
...
for(i in 1:N.sim){
  for(j in 1:nrow(pDP_U_U_26[[i]])){
    rel_bias_relDP_U_U_26[[i]] <- round((listaDP_overlap_U_U_26[[i]]$`rel_effDP_U_U_26[[i]]` - p_overlapDP_U_U_26[[i]]$true_eff ),2)
...
for(i in 1:N.sim){
  dataDP_U_U_26[[i]] <- list(DP = listaDP_overlap_U_U_26[[i]]$`rel_effDP_U_U_26[[i]]` , True = p_overlapDP_U_U_26[[i]]$true_eff)
...
write.csv(DP_bin_jags_U_U_26, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_UNIF_UNIF26_ResultsDP_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(listaDP_U_U_26, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_UNIF_UNIF26_Relative_eff_ResultsDP_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(listaDP_rel_eff_U_U_26, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_UNIF_UNIF26_Bias_Relative_eff_ResultsDP_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(listaDP_weights_U_U_26, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_UNIF_UNIF26_Weights_in_clusters_DP_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=TRUE )  
write.csv(Overlap_DPmodel_U_U_26, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_UNIF_UNIF26_Overlap_DPmodel_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=TRUE )  
...
# ############ TO EXTRACT THE RELATIVE EFFECTS ####
max_rowsDP_U_G_n <- max(sapply(listaDP1_U_G_n, function(x) dim(x)[1]))
common_colnamesDP_U_G_n <- colnames(listaDP1_U_G_n[[1]])  
# 
for (i in 2:length(listaDP1_U_G_n)) {
  if (!identical(colnames(listaDP1_U_G_n[[i]]), common_colnamesDP_U_G_n)) {
    stop("Column names do not match between lists.")
  }
}
for (i in 1:length(listaDP1_U_G_n)) {
  current_rowsDP_U_G_n <- dim(listaDP1_U_G_n[[i]])[1]
  if (current_rowsDP_U_G_n < max_rowsDP_U_G_n) {
    extra_rowsDP_U_G_n <- max_rowsDP_U_G_n - current_rowsDP_U_G_n
    extra_dataDP_U_G_n <- matrix(NA, nrow = extra_rowsDP_U_G_n, ncol = ncol(listaDP1_U_G_n[[i]]))
    colnames(extra_dataDP_U_G_n) <- common_colnamesDP_U_G_n  
    listaDP1_U_G_n[[i]] <- rbind(listaDP1_U_G_n[[i]], extra_dataDP_U_G_n)
  } else if (current_rowsDP_U_G_n > max_rowsDP_U_G_n) {
    listaDP1_U_G_n[[i]] <- listaDP1_U_G_n[[i]][1:max_rowsDP_U_G_n, ]
  }
}
# 
max_rowsDPw_U_G_n <- max(sapply(listaDP_weights_U_G_n, function(x) dim(x)[1]))
common_colnamesDPw_U_G_n <- colnames(listaDP_weights_U_G_n[[1]])  
# 
for (i in 2:length(listaDP_weights_U_G_n)) {
  if (!identical(colnames(listaDP_weights_U_G_n[[i]]), common_colnamesDPw_U_G_n)) {
    stop("Column names do not match between lists.")
  }
}
for (i in 1:length(listaDP_weights_U_G_n)) {
  current_rowsDPw_U_G_n <- dim(listaDP_weights_U_G_n[[i]])[1]
  if (current_rowsDPw_U_G_n < max_rowsDPw_U_G_n) {
    extra_rowsDPw_U_G_n <- max_rowsDPw_U_G_n - current_rowsDPw_U_G_n
    extra_dataDPw_U_G_n <- matrix(NA, nrow = extra_rowsDPw_U_G_n, ncol = ncol(listaDP_weights_U_G_n[[i]]))
    colnames(extra_dataDPw_U_G_n) <- common_colnamesDPw_U_G_n  
    listaDP_weights_U_G_n[[i]] <- rbind(listaDP_weights_U_G_n[[i]], extra_dataDPw_U_G_n)
  } else if (current_rowsDPw_U_G_n > max_rowsDPw_U_G_n) {
    listaDP_weights_U_G_n[[i]] <- listaDP_weights_U_G_n[[i]][1:max_rowsDPw_U_G_n, ]
  }
}
...
subtr_U_G_n <- length(which(dist.condDP_U_G_n < studies_zero_2arms))
number_U_G_n = studies_zero_2arms - subtr_U_G_n


listaDP_overlap_U_G_n = listaDP_U_G_n[-number_U_G_n]  
p_overlapDP_U_G_n = pDP_U_G_n[-number_U_G_n]
N.sim = length(listaDP_overlap_U_G_n)

abs_bias_relDP_U_G_n <- list()
mean_abs_bias_relDP_U_G_n <- list()
for(i in 1:N.sim){
  for(j in 1:nrow(pDP_U_G_n[[i]])){
    abs_bias_relDP_U_G_n[[i]] <- round(abs(listaDP_overlap_U_G_n[[i]]$`rel_effDP_U_G_n[[i]]` - p_overlapDP_U_G_n[[i]]$true_eff ),2)
...
for(i in 1:N.sim){
  for(j in 1:nrow(pDP_U_G_n[[i]])){
    rel_bias_relDP_U_G_n[[i]] <- round((listaDP_overlap_U_G_n[[i]]$`rel_effDP_U_G_n[[i]]` - p_overlapDP_U_G_n[[i]]$true_eff ),2)
...
for(i in 1:N.sim){
  dataDP_U_G_n[[i]] <- list(DP = listaDP_overlap_U_G_n[[i]]$`rel_effDP_U_G_n[[i]]` , True = p_overlapDP_U_G_n[[i]]$true_eff)
...
write.csv(DP_bin_jags_U_G_n, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_UNIF_gamma_ns_ResultsDP_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(listaDP_U_G_n, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_UNIF_gamma_ns_Relative_eff_ResultsDP_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(listaDP_rel_eff_U_G_n, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_UNIF_gamma_ns_Bias_Relative_eff_ResultsDP_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(listaDP_weights_U_G_n, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_UNIF_gamma_ns_Weights_in_clusters_DP_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=TRUE )  
write.csv(Overlap_DPmodel_U_G_n, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_UNIF_gamma_ns_Overlap_DPmodel_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=TRUE )  
...
# ############ TO EXTRACT THE RELATIVE EFFECTS ####
max_rowsDP_U_U_51 <- max(sapply(listaDP1_U_U_51, function(x) dim(x)[1]))
common_colnamesDP_U_U_51 <- colnames(listaDP1_U_U_51[[1]])  
# 
for (i in 2:length(listaDP1_U_U_51)) {
  if (!identical(colnames(listaDP1_U_U_51[[i]]), common_colnamesDP_U_U_51)) {
    stop("Column names do not match between lists.")
  }
}
for (i in 1:length(listaDP1_U_U_51)) {
  current_rowsDP_U_U_51 <- dim(listaDP1_U_U_51[[i]])[1]
  if (current_rowsDP_U_U_51 < max_rowsDP_U_U_51) {
    extra_rowsDP_U_U_51 <- max_rowsDP_U_U_51 - current_rowsDP_U_U_51
    extra_dataDP_U_U_51 <- matrix(NA, nrow = extra_rowsDP_U_U_51, ncol = ncol(listaDP1_U_U_51[[i]]))
    colnames(extra_dataDP_U_U_51) <- common_colnamesDP_U_U_51  
    listaDP1_U_U_51[[i]] <- rbind(listaDP1_U_U_51[[i]], extra_dataDP_U_U_51)
  } else if (current_rowsDP_U_U_51 > max_rowsDP_U_U_51) {
    listaDP1_U_U_51[[i]] <- listaDP1_U_U_51[[i]][1:max_rowsDP_U_U_51, ]
  }
}
# 
max_rowsDPw_U_U_51 <- max(sapply(listaDP_weights_U_U_51, function(x) dim(x)[1]))
common_colnamesDPw_U_U_51 <- colnames(listaDP_weights_U_U_51[[1]])  
# 
for (i in 2:length(listaDP_weights_U_U_51)) {
  if (!identical(colnames(listaDP_weights_U_U_51[[i]]), common_colnamesDPw_U_U_51)) {
    stop("Column names do not match between lists.")
  }
}
for (i in 1:length(listaDP_weights_U_U_51)) {
  current_rowsDPw_U_U_51 <- dim(listaDP_weights_U_U_51[[i]])[1]
  if (current_rowsDPw_U_U_51 < max_rowsDPw_U_U_51) {
    extra_rowsDPw_U_U_51 <- max_rowsDPw_U_U_51 - current_rowsDPw_U_U_51
    extra_dataDPw_U_U_51 <- matrix(NA, nrow = extra_rowsDPw_U_U_51, ncol = ncol(listaDP_weights_U_U_51[[i]]))
    colnames(extra_dataDPw_U_U_51) <- common_colnamesDPw_U_U_51  
    listaDP_weights_U_U_51[[i]] <- rbind(listaDP_weights_U_U_51[[i]], extra_dataDPw_U_U_51)
  } else if (current_rowsDPw_U_U_51 > max_rowsDPw_U_U_51) {
    listaDP_weights_U_U_51[[i]] <- listaDP_weights_U_U_51[[i]][1:max_rowsDPw_U_U_51, ]
  }
}
...
subtr_U_U_51 <- length(which(dist.condDP_U_U_51 < studies_zero_2arms))
number_U_U_51 = studies_zero_2arms - subtr_U_U_51


listaDP_overlap_U_U_51 = listaDP_U_U_51[-number_U_U_51]  
p_overlapDP_U_U_51 = pDP_U_U_51[-number_U_U_51]
N.sim = length(listaDP_overlap_U_U_51)

abs_bias_relDP_U_U_51 <- list()
mean_abs_bias_relDP_U_U_51 <- list()
for(i in 1:N.sim){
  for(j in 1:nrow(pDP_U_U_51[[i]])){
    abs_bias_relDP_U_U_51[[i]] <- round(abs(listaDP_overlap_U_U_51[[i]]$`rel_effDP_U_U_51[[i]]` - p_overlapDP_U_U_51[[i]]$true_eff ),2)
...
for(i in 1:N.sim){
  for(j in 1:nrow(pDP_U_U_51[[i]])){
    rel_bias_relDP_U_U_51[[i]] <- round((listaDP_overlap_U_U_51[[i]]$`rel_effDP_U_U_51[[i]]` - p_overlapDP_U_U_51[[i]]$true_eff ),2)
...
for(i in 1:N.sim){
  dataDP_U_U_51[[i]] <- list(DP = listaDP_overlap_U_U_51[[i]]$`rel_effDP_U_U_51[[i]]` , True = p_overlapDP_U_U_51[[i]]$true_eff)
...
write.csv(DP_bin_jags_U_U_51, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_UNIF_UNIF51_ResultsDP_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(listaDP_U_U_51, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_UNIF_UNIF51_Relative_eff_ResultsDP_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(listaDP_rel_eff_U_U_51, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_UNIF_UNIF51_Bias_Relative_eff_ResultsDP_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(listaDP_weights_U_U_51, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_UNIF_UNIF51_Weights_in_clusters_DP_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=TRUE )  
write.csv(Overlap_DPmodel_U_U_51, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\DP MODEL\\rev_UNIF_UNIF51_Overlap_DPmodel_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=TRUE )  
...
######### AVERAGE ABSOLUTE BIAS OF skew ##########
abs_bias_skewSNjags.bin_HN <- c()
for (i in 1:N.sim){
  abs_bias_skewSNjags.bin_HN[i] <- abs( data_SN_HN$skewSN_HN[i] - 0.785) 
}
avg_abs_bias_skewSNjags.bin_HN <- round(mean(abs_bias_skewSNjags.bin_HN), 2)

########## AVERAGE DIFFERENCES OF skew ##########
rel_bias_skewSNjags.bin_HN <- c()
for (i in 1:N.sim){
  rel_bias_skewSNjags.bin_HN[i] <- ( data_SN_HN$skewSN_HN[i] - 0.785) 
}
avg_rel_bias_skewSNjags.bin_HN <- round(mean(rel_bias_skewSNjags.bin_HN), 2)


############### MSE OF skew  ##################
MSE_skewSNjags.bin_HN <- c()
for (i in 1:N.sim){
  MSE_skewSNjags.bin_HN[i] <- RMSE(estimate = data_SN_HN$skewSN_HN[i],
                                   parameter = 0.785,
                                   type = "RMSE",
                                   MSE = TRUE,           
                                   percent = FALSE,
...
avg_MSE_skewSNjags.bin_HN <-  round(mean(MSE_skewSNjags.bin_HN),4)

########## COVERAGE OF skew ##########
interval_contains_true_mean <- function(i) { 
  ( 0.785 >= data_SN_HN$LBskewSN_HN[i]) && ( 0.785 <=  data_SN_HN$UBskewSN_HN[i])
}
interval_contains_true_mean(i)

coverage_skewSNjags.bin_HN <- c()
for (i in 1:N.sim){0
  if (interval_contains_true_mean(i) == TRUE){
    coverage_skewSNjags.bin_HN[i] = 1
  }
  else {
    coverage_skewSNjags.bin_HN[i]=0
  }
  print(coverage_skewSNjags.bin_HN[i])
}
coverage_skewSNjags.bin_HN
cover_skewSNjags.bin_HN <- round(mean(coverage_skewSNjags.bin_HN)*100,2)

######################## ABSOLUTE BIAS OF RELATIVE EFFECTS ##################
######################## BIAS OF RELATIVE EFFECTS ##################
subtr_HN <- length(which(dist.condSN_HN < studies_zero_2arms))
number_HN = studies_zero_2arms - subtr_HN


listaSN_overlap_HN = delta_SN_HN[-number_HN]  
p_overlapSN_HN = pSN_HN[-number_HN]
N.sim = length(listaSN_overlap_HN)

abs_bias_relSN_HN <- list()
mean_abs_bias_relSN_HN <- list()
for(i in 1:N.sim){
  for(j in 1:nrow(pSN_HN[[i]])){
    abs_bias_relSN_HN[[i]] <- round(abs(listaSN_overlap_HN[[i]]$median - p_overlapSN_HN[[i]]$true_eff),2)
...
for(i in 1:N.sim){
  for(j in 1:nrow(pSN_HN[[i]])){
    rel_bias_relSN_HN[[i]] <- round((listaSN_overlap_HN[[i]]$median - p_overlapSN_HN[[i]]$true_eff),2)
...
overlapSN_HN <- list()
for(i in 1:N.sim){
  dataSN1_HN[[i]] <- list(SN = listaSN_overlap_HN[[i]]$median , True = p_overlapSN_HN[[i]]$true_eff)
...
SNormal_HN = cbind(data_SN_HN, abs_biasSNjags.bin_HN, rel_biasSNjags.bin_HN, 
                   ...
                   avg_abs_bias_skewSNjags.bin_HN, avg_rel_bias_skewSNjags.bin_HN, 
                   avg_MSE_skewSNjags.bin_HN,cover_skewSNjags.bin_HN,
                   len_mu_SN_HN ,
                   len_PI_SN_HN,
                   RhatSN_out_HN)
...
write.csv(SNormal_HN, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\SKEW NORMAL MODEL\\rev_HN_ResultsSN_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(deltaSN_HN, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\SKEW NORMAL MODEL\\rev_HN_Relative_eff_ResultsSN_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv" ) 
write.csv(listaSN_rel_eff_HN, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\SKEW NORMAL MODEL\\rev_HN_Bias_Relative_eff_ResultsSN_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv" ) 
write.csv(listaSN_HN, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\SKEW NORMAL MODEL\\rev_HN_Skew_ResultsSN_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv" ) 
write.csv(Overlap_SNmodel_HN, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\SKEW NORMAL MODEL\\rev_HN_Overlap_SNmodel_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv" ) 
...
########### AVERAGE ABSOLUTE BIAS OF skew ##########
abs_bias_skewSNjags.bin_U <- c()
for (i in 1:N.sim){
  abs_bias_skewSNjags.bin_U[i] <- abs( data_SN_U$skewSN_U[i] - 0.785) 
}
avg_abs_bias_skewSNjags.bin_U <- round(mean(abs_bias_skewSNjags.bin_U), 2)

########## AVERAGE DIFFERENCES OF skew ##########
rel_bias_skewSNjags.bin_U <- c()
for (i in 1:N.sim){
  rel_bias_skewSNjags.bin_U[i] <- ( data_SN_U$skewSN_U[i] - 0.785) 
}
avg_rel_bias_skewSNjags.bin_U <- round(mean(rel_bias_skewSNjags.bin_U), 2)

############### MSE OF skew  ##################
MSE_skewSNjags.bin_U <- c()
for (i in 1:N.sim){
  MSE_skewSNjags.bin_U[i] <- RMSE(estimate = data_SN_U$skewSN_U[i],
                                  parameter = 0.785,
                                  type = "RMSE",
                                  MSE = TRUE,           
                                  percent = FALSE,
...
avg_MSE_skewSNjags.bin_U <-  round(mean(MSE_skewSNjags.bin_U),4)

########## COVERAGE OF skew ##########
interval_contains_true_mean <- function(i) { 
  ( 0.785 >= data_SN_U$LBskewSN_U[i]) && ( 0.785 <=  data_SN_U$UBskewSN_U[i])
}
interval_contains_true_mean(i)

coverage_skewSNjags.bin_U <- c()
for (i in 1:N.sim){0
  if (interval_contains_true_mean(i) == TRUE){
    coverage_skewSNjags.bin_U[i] = 1
  }
  else {
    coverage_skewSNjags.bin_U[i]=0
  }
  print(coverage_skewSNjags.bin_U[i])
}
coverage_skewSNjags.bin_U
cover_skewSNjags.bin_U <- round(mean(coverage_skewSNjags.bin_U)*100,2)


######################## ABSOLUTE BIAS OF RELATIVE EFFECTS ##################
######################## BIAS OF RELATIVE EFFECTS ##################
subtr_U <- length(which(dist.condSN_U < studies_zero_2arms))
number_U = studies_zero_2arms - subtr_U


listaSN_overlap_U = delta_SN_U[-number_U]  
p_overlapSN_U = pSN_U[-number_U]
N.sim = length(listaSN_overlap_U)

abs_bias_relSN_U <- list()
mean_abs_bias_relSN_U <- list()
for(i in 1:N.sim){
  for(j in 1:nrow(pSN_U[[i]])){
    abs_bias_relSN_U[[i]] <- round(abs(listaSN_overlap_U[[i]]$median - p_overlapSN_U[[i]]$true_eff),2)
...
for(i in 1:N.sim){
  for(j in 1:nrow(pSN_U[[i]])){
    rel_bias_relSN_U[[i]] <- round((listaSN_overlap_U[[i]]$median - p_overlapSN_U[[i]]$true_eff),2)
...
overlapSN_U <- list()
for(i in 1:N.sim){
  dataSN1_U[[i]] <- list(SN = listaSN_overlap_U[[i]]$median , True = p_overlapSN_U[[i]]$true_eff)
...
SNormal_U = cbind(data_SN_U, abs_biasSNjags.bin_U, rel_biasSNjags.bin_U, 
                  ...
                  avg_abs_bias_skewSNjags.bin_U, avg_rel_bias_skewSNjags.bin_U, 
                  avg_MSE_skewSNjags.bin_U,cover_skewSNjags.bin_U,
                  len_mu_SN_U ,
                  len_PI_SN_U ,
                  RhatSN_out_U)
...
write.csv(SNormal_U, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\SKEW NORMAL MODEL\\rev_U_ResultsSN_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(deltaSN_U, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\SKEW NORMAL MODEL\\rev_U_Relative_eff_ResultsSN_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv" ) 
write.csv(listaSN_rel_eff_U, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\SKEW NORMAL MODEL\\rev_U_Bias_Relative_eff_ResultsSN_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv" ) 
write.csv(listaSN_U, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\SKEW NORMAL MODEL\\rev_U_Skew_ResultsSN_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv" ) 
write.csv(Overlap_SNmodel_U, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\SKEW NORMAL MODEL\\rev_U_Overlap_SNmodel_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv" ) 
...
######################## ABSOLUTE BIAS OF RELATIVE EFFECTS ##################
######################## BIAS OF RELATIVE EFFECTS ##################
subtr_HN <- length(which(dist.condt_HN < studies_zero_2arms))
number_HN = studies_zero_2arms - subtr_HN

listat_overlap_HN = delta_t_distr_HN[-number_HN]  
p_overlapt_HN = pT_HN[-number_HN]
N.sim = length(listat_overlap_HN)

abs_bias_relt_distr_HN <- list()
mean_abs_bias_relt_distr_HN <- list()
for(i in 1:N.sim){
  for(j in 1:nrow(pT_HN[[i]])){
    abs_bias_relt_distr_HN[[i]] <- round(abs(listat_overlap_HN[[i]]$median - p_overlapt_HN[[i]]$true_eff),2)
...
for(i in 1:N.sim){
  for(j in 1:nrow(pT_HN[[i]])){
    rel_bias_relt_distr_HN[[i]] <- round((listat_overlap_HN[[i]]$median - p_overlapt_HN[[i]]$true_eff),2)
...
overlapt_HN <- list()
for(i in 1:N.sim){
  datat_HN[[i]] <- list(t_distr = listat_overlap_HN[[i]]$median , True = p_overlapt_HN[[i]]$true_eff)
...
write.csv(t_distr_HN, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\T MODEL\\rev_HN_Results_t_distr_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(delta_tdistr_HN, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\T MODEL\\rev_HN_Relative_eff_Results_t_distr_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv" ) 
write.csv(listat_distr_rel_eff_HN, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\T MODEL\\rev_HN_Bias_Relative_eff_Results_t_distr_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv" ) 
write.csv(Overlap_tdistr_HN, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\T MODEL\\rev_HN_Overlap_tdistr_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv" ) 
...
######################## ABSOLUTE BIAS OF RELATIVE EFFECTS ##################
######################## BIAS OF RELATIVE EFFECTS ##################
subtr_U <- length(which(dist.condt_U < studies_zero_2arms))
number_U = studies_zero_2arms - subtr_U

listat_overlap_U = delta_t_distr_U[-number_U]  
p_overlapt_U = pT_U[-number_U]
N.sim = length(listat_overlap_U)

abs_bias_relt_distr_U <- list()
mean_abs_bias_relt_distr_U <- list()
for(i in 1:N.sim){
  for(j in 1:nrow(pT_U[[i]])){
    abs_bias_relt_distr_U[[i]] <- round(abs(listat_overlap_U[[i]]$median - p_overlapt_U[[i]]$true_eff),2)
...
for(i in 1:N.sim){
  for(j in 1:nrow(pT_U[[i]])){
    rel_bias_relt_distr_U[[i]] <- round((listat_overlap_U[[i]]$median - p_overlapt_U[[i]]$true_eff),2)
...
overlapt_U <- list()
for(i in 1:N.sim){
  datat_U[[i]] <- list(t_distr = listat_overlap_U[[i]]$median , True = p_overlapt_U[[i]]$true_eff)
...
write.csv(t_distr_U, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\T MODEL\\rev_U_Results_t_distr_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
write.csv(delta_tdistr_U, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\T MODEL\\rev_U_Relative_eff_Results_t_distr_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv" ) 
write.csv(listat_distr_rel_eff_U, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\T MODEL\\rev_U_Bias_Relative_eff_Results_t_distr_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv" ) 
write.csv(Overlap_tdistr_U, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\T MODEL\\rev_U_Overlap_tdistr_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv" ) 
...
write.csv(fr_normal.bin , "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\freq_normal.bin\\rev_Results_freq_N_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
...
stud_eff = list()
for(i in 1:N.sim){
  stud_eff[[i]] = as.data.frame(blup( freq_norm[[i]], level = 95 ))
...
write.csv(overlap_normfr , "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\freq_normal\\rev_Overlap_freq_N_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  

####################### ABSOLUTE BIAS OF RELATIVE EFFECTS ##################
# ####################### BIAS OF RELATIVE EFFECTS ##################
abs_bias_relnormfr_distr <- list()
mean_abs_bias_relnormfr_distr <- numeric(N.sim)

for (i in 1:N.sim) {
  abs_bias_relnormfr_distr[[i]] <- numeric(nrow(p[[i]]))
  for (j in 1:nrow(p[[i]])) {
    abs_bias_relnormfr_distr[[i]][j] <- abs(stud_eff[[i]]$pred[j] - p[[i]]$true_eff[j])
  }
...
write.csv(mean_abs_bias_of_mean_abs_bias_relnormfr_distr , "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\freq_normal\\rev_Mean_abs_bias_Rel_eff_freq_N_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  


# ####################### AVERAGE DIFFERENCES OF RELATIVE EFFECTS ##################
# ####################### BIAS OF RELATIVE EFFECTS ##################
rel_bias_relnormfr_distr <- list()
mean_rel_bias_relnormfr_distr <- numeric(N.sim)

for (i in 1:N.sim) {
  rel_bias_relnormfr_distr[[i]] <- numeric(nrow(p[[i]]))
  for (j in 1:nrow(p[[i]])) {
    rel_bias_relnormfr_distr[[i]][j] <- (stud_eff[[i]]$pred[j] - p[[i]]$true_eff[j])
  }
...
write.csv(mean_rel_bias_of_mean_rel_bias_relnormfr_distr , "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\freq_normal\\rev_Mean_rel_bias_Rel_eff_freq_N_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  

# #### TO SAVE THE RELATIVE EFFECTS #########
max_rowsNfr <- max(sapply(stud_eff, function(x) dim(x)[1]))
# #
#
common_colnamesNfr <- colnames(stud_eff[[1]])
# #
for (i in 2:length(stud_eff)) {
  if (!identical(colnames(stud_eff[[i]]), common_colnamesNfr)) {
    stop("Column names do not match between lists.")
  }
}
# #
#
for (i in 1:length(stud_eff)) {
  current_rowsNfr <- dim(stud_eff[[i]])[1]
  if (current_rowsNfr < max_rowsNfr) {
    extra_rowsNfr <- max_rowsNfr - current_rowsNfr
    extra_dataNfr <- matrix(NA, nrow = extra_rowsNfr, ncol = ncol(stud_eff[[i]]))
    colnames(extra_dataNfr) <- common_colnamesNfr
    stud_eff[[i]] <- rbind(stud_eff[[i]], extra_dataNfr)
  } else if (current_rowsNfr > max_rowsNfr) {
    stud_eff[[i]] <- stud_eff[[i]][1:max_rowsNfr, ]
  }
}


write.csv(stud_eff , "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\freq_normal\\rev_Rel_eff_freq_N_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )
...
write.csv(fr_normal, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\freq_normal\\rev_Results_freq_N_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
...
write.csv(metaplus_t, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\freq_tmodel\\rev_Results_metaplust_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
...
write.csv(metaplus_mix, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\freq_mixture\\rev_Results_metaplusmix_jags_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv",row.names=FALSE )  
...
write.csv(matrix, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\ALL MODELS\\rev_Matrix_All_models_SNormal_bin_data_k=26_mu=0_tau2=2.63.csv" ) 
...
write.csv(matrix1, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\ALL MODELS\\rev_Matrix_Bias_Rel_eff_SNormal_data_k=26_mu=0_tau2=2.63.csv" ) 
...
write.csv(matrix2, "C:\\Users\\Lela Panag\\Desktop\\SIMULATION SCENARIOS FINAL\\SKEW NORMAL DATA\\ALL MODELS\\Matrix_prec_SNormal_data_k=26_mu=0_tau2=2.63.csv" ) 
